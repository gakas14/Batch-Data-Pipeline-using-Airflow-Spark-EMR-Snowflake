...
dag = DAG(
    dag_id="MTA_ETL_JOB", default_args=args, schedule_interval=None
)



client = boto3.client('emr', region_name='us-east-1',aws_access_key_id=credentials.SecretId,aws_secret_access_key=credentials.SecretKey)

def create_emr_cluster():
  cluster_id = client.run_job_flow(
    Name="transient_EMR_job",
    Instances={
    'InstanceGroups': [
    {
    'Name': "Master",
    ...
    'Ec2SubnetId': 'subnet-0f3e699aa6a360dad',
    },
    LogUri="s3://ak-emr-logs/",
    ReleaseLabel= 'emr-5.33.0',
    BootstrapActions=[],
    VisibleToAllUsers=True,
    JobFlowRole="EC2_emr_s3_full_access",
...
with dag:
  ...
  op_args=['{{ ti.xcom_pull("create_emr_cluster")["JobFlowId"]}}','s3://us-east-1.elasticmapreduce/libs/script-runner/script-runner.jar',['s3://gakas-mta-etl-job/scripts/ingest.sh']],
  dag=dag, 
  )
  poll_ingest_layer = PythonOperator(
  task_id='poll_ingest_layer',
  ...
  op_args=['{{ ti.xcom_pull("create_emr_cluster")["JobFlowId"]}}','command-runner.jar',[ 'spark-submit',
            '--master', 'yarn',
            '--deploy-mode', 'cluster',
            's3://gakas-mta-etl-job/scripts/transformation.py']],
  dag=dag, 
  )
  poll_transform_layer = PythonOperator(
  task_id='poll_transform_layer',
  python_callable=wait_for_step_to_complete,
  ...
  snowflake_load=SnowflakeOperator(
		...
		sql="""ALTER EXTERNAL TABLE emr_job_db.PUBLIC.totat_daily_ridership_table REFRESH""" ,
		snowflake_conn_id="snowflake_conn"
	)

create_emr_cluster >> ingest_layer >> poll_ingest_layer >> transform_layer >> poll_transform_layer >> terminate_emr_cluster >> snowflake_load
